...
| V37 | home | **TREE CONNECTORS — STILL SKIPPED (3rd ask). ⚠ B did the EASY half (corner ✓ badges, `2f25d4f`) and AGAIN skipped the CONNECTORS.** This is the row's load-bearing part: draw the unlock-graph edges from `modes.json` — a **gold** arrow from each node's `unlock.by` parent (vertical chain) + a **purple** branch from its `unlock.mastery` parent (to the part within a topic), arrowhead INTO the child. **If you genuinely cannot route graph edges, FLAG IT (→ owner accept) — do NOT silently ship badges-only a 4th time.** Badges ✓ done. | med | B | OPEN | |
...
| V44 | inventory / heroes / arena | **MIXED TYPE SYSTEM — ⭐ KEYSTONE, NOT YET ATTEMPTED (gates every inventory + arena sign-off).** web is NOT all-mono: **proportional bold sans for CONTENT NAMES** (topic / region / hero / item names), pixel/mono only for NUMBERS + chrome. B's V25 fix over-corrected to all-mono. Until this lands, inventory-loot/topics/awards/events/codex + arena-map all stay EXAMINE (they're otherwise close). Do this NEXT — it's the highest-leverage open row. | high | B | OPEN | |
