...
| V37 | home | **TREE CONNECTORS — PARTIAL (v3, `09590b6`). (Babysitter correction: I marked this RESOLVED on a quick read, then re-checked the raw render — it's NOT done.)** B routed a real gold VERTICAL chain down the left column with arrowheads (genuine progress, beyond the old spine), but the **horizontal SIBLING arrows** (axb→n÷k→+9→1n within a row) and the **purple `unlock.mastery` branches** are still missing — nodes in columns 2–4 float. Finish: per-row sibling links + the purple branches so every node has its real incoming edge. | med | B | OPEN | |
