...
| V37 | home | **TREE CONNECTORS — 🔴 ESCALATED to SOLE-TASK (half-done 3×: badges, then "thicker spine", still NO graph edges).** wave-9 "v2" just thickened the central spine — still no per-node `unlock.by` arrows + no purple `unlock.mastery` branches; the tree still reads as a grid, not a connected graph. **Per the NO-DEFER rule this is now blocking: do ONLY this until it lands** — route the actual edges from `modes.json` (gold arrow parent→child along `unlock.by`; purple branch along `unlock.mastery`; arrowhead INTO child). **OR, if brickmap genuinely cannot route graph edges, say so explicitly → owner ACCEPT.** No 4th thicker-spine. | med | B | OPEN | |
...
| V44 | inventory / heroes / arena | **MIXED TYPE — attempted (wave-9 `7f4bb3f`, InstrumentSans) but DOESN'T HIT PARITY (3 verify agents FAILed it).** B swapped the font (names ARE technically variable-width now) but it still **reads as a blocky pixel face, not web's smooth proportional sans**, AND B colored the names **GREEN not WHITE** (screen goes monochromatic green; web = white names / green accents), AND oversized them (fewer rows fit) + the card borders went faint. Fix: names must READ as a smooth proportional sans, **WHITE**, at web's size, with the green reserved to bars/counts; verify with your OWN gate that it doesn't read mono/green. ⭐ still the keystone — gates all inventory + arena. | high | B | OPEN | |
