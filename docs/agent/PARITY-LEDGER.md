...
| ~~V37a~~ | home | ~~boxes packed edge-to-edge, no room for connectors~~ → **RESOLVED** (wave-13 `96aa5db`, owner's placement-first call): topic-box parts now SPREAD to fill the row with gaps (per the measured 60px/38px spec); verified on the raw render. | high | B | RESOLVED | verified 06-29 |
| ~~V37~~ | home | ~~connectors (6 attempts, blocked on placement)~~ → **RESOLVED** (wave-13 `96aa5db`, once V37a opened the gaps): the full HORIZONTAL sibling mesh now connects every node (axb→n÷k→+9→1n etc.) + the vertical gold chain + 2-part rows span their gap + ✓ badges. Reads as a connected graph. Placement-first was the unlock. | med | B | RESOLVED | verified 06-29 |
