...
| V28 | heroes / hero-detail | **RANK GLYPH still wrong (2 attempts).** Web = filled ★. B's "v2 star" (`a7c1a8d`) renders a lumpy yellow blob/splat sprite — still not a recognisable ★ — AND it now **overlaps/occludes the trailing FOC stat chip (a NEW layout collision)**. Render a clean filled ★ that doesn't collide with the stats. | med | B | OPEN | |
...
| V36 | home | **COIN-HOARD still sparse (2nd attempt).** B's `a7c1a8d` "denser band" did NOT land — the agent still sees a thin gold fleck-band with no side "wings", vs web's dense pixel coin-pile filling ~bottom 45% with rising side wings. Match seedHoard density/extent + the side wings. | med | B | OPEN | |
...
> ⚠️ **agent-review gate progress (2026-06-28):** every old `examine-PASSED` mark is VOID — a screen is signed only
> on an agent-PASS. **SIGNED so far (agent-PASS): `summary` ✓, `drill` ✓** (wave-2 `a7c1a8d`; minor residuals V40/V41
> only). **Still agent-FAIL:** `home` (V36 hoard / V37 edges / V38 nav-glyphs), `heroes-partial` (V28 star+collision /
> V30 chips / V31 tap-for-details), `hero-detail` (V29 list-order = N2, mine). `results`/`heroes` = clean-ish EXAMINE
> pending a re-pass. **The remaining ~13 rendered screens still need their FIRST agent review** (event-play, arena-*,
> inventory-*, hero-detail-{arcane,cunning}, home-{fresh,midprogress}, settings, audio).
