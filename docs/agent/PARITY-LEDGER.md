...
| V36 | home | **COIN-HOARD — closer, not there (5 attempts: sparse→sparse→solid→…→textured).** `2074b86` is now a TEXTURED pile (coin bricks + dark gaps — good, the texture finally landed) but it sits too LOW/shallow (~bottom 30% vs web's ~45%) and the rising side "WINGS" are weak/short (web's climb to mid-screen). Raise the coverage + extend the side wings. | med | B | OPEN | |
