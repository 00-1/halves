...
| V37a | home | **TOPIC-BOX PLACEMENT — DO THIS FIRST (owner, 2026-06-29: "get the placing of the topic boxes right first, then the connectors — there's no real space for the connectors now").** B packs the row nodes edge-to-edge (≈0 gap); web spreads them. **MEASURED web layout (DOM, tree width 360px device): 4-part row → 4 nodes each 60px wide × 52px tall, INTER-PART GAPS 39/38/39px; 2-part row → 96px nodes, 116px gap; 1-part row → one 96px node centred.** Lay the parts out to FILL the row width with those gaps (justify-between within 360px). THE GAPS ARE WHERE THE CONNECTORS GO. V37 connectors are BLOCKED on this. | high | B | OPEN | |
| V37 | home | **TREE CONNECTORS — BLOCKED on V37a (placement).** After 5 attempts the horizontal sibling mesh won't land because there's no gap to draw into (see V37a). Once the ~38px inter-part gaps exist: draw the purple `unlock.mastery` branch in each gap + the gold `unlock.by` vertical arrows between rows, arrowhead INTO child, every node with a real incoming edge. **If it still can't route after placement is fixed, FLAG for owner ACCEPT** (vertical-chain-only is a minor 1-element gap). | med | B | OPEN | |
...
| V55 | inventory rows (all tabs) | **✓ CHECK PADDING (owner, 2026-06-29):** the row ✓ checkmarks sit cramped against / clipping the right card edge with uneven padding (agents flagged it too). Give the ✓ equal/consistent padding from the card edge — an instance of V52/V53. | low | B | OPEN | |
